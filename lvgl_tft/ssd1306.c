 ...
 *  STATIC PROTOTYPES
 **********************/
static esp_err_t send_data(lv_disp_drv_t *disp_drv, void *bytes, size_t bytes_len);
static esp_err_t send_pixels(lv_disp_drv_t *disp_drv, void *color_buffer, size_t buffer_len);
...
void ssd1306_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    ESP_ERROR_CHECK(send_data(disp_drv, conf, sizeof(conf)));
    ESP_ERROR_CHECK(send_pixels(disp_drv, color_p, OLED_COLUMNS * (1 + row2 - row1)));
...
esp_err_t ssd1306_sleep_in(void)
{
    uint8_t conf[] = {
        OLED_CONTROL_BYTE_CMD_STREAM,
...
esp_err_t ssd1306_sleep_out(void)
{
    uint8_t conf[] = {
        OLED_CONTROL_BYTE_CMD_STREAM,
        OLED_CMD_DISPLAY_ON};
...
static esp_err_t send_data(lv_disp_drv_t *disp_drv, void *bytes, size_t bytes_len)
{
    ...
    esp_err_t err = i2c_master_cmd_begin(DISP_I2C_PORT, cmd, pdMS_TO_TICKS(10));
